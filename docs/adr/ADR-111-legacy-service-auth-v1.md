...
**Status:** **Accepted direction** — **ratified as rev 2 by the Accountable on 2026-09-17**. This is now the governing cross-repo service-authentication contract; TD-13 is no longer TENTATIVE. Implementation has not started.
**Date:** 2026-09-17
...
- **Ratified, and still not implemented.** Ratification settles the contract; it authorizes no code. The Gu OS verifier, the raw-body handling and the key registry are SL-5 and SL-6 work, and **C1 additionally waits on [ADR-112](ADR-112-cross-repo-integration-events.md)**, which is not ratified. C6's payloads were always implementable; with this ratified, C6's authenticated path is specifiable too.
