...
**Rate limiting** applies to rejected requests as well as accepted ones, and sits outside the signature contract. On Gu OS C2 this is the existing process-local `rateLimit` helper (`apps/web/src/lib/public-rate-limit.ts`). Because the limiter runs **before** authentication, the presented `x-guos-key-id` is classified first: missing, malformed, and well-formed-but-unknown identifiers share finite class buckets and are **not** stored or logged as raw attacker-controlled values; known (provisioned) key ids remain per-key, including bad-signature attempts. It is not durable across application instances. A shared durable limiter is a platform prerequisite, not part of this protocol.
