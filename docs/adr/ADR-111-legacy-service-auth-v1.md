...
3. **Legacy source scope.** The key record names the `source_system` and the Traditional Gu owner/source refs the caller may speak for. An event about an out-of-scope owner is `403`. For the pilot a single owner ref per key is sufficient; the field is a set so that widening later does not change the protocol. **An empty `ownerRefs` set authorizes no owner.** A signed payload `legacy_owner_ref` is a consistency assertion only: it is never authorization by itself, and omitting it does not widen the key. The actual owner/source of the requested Lead is derived from **server-observed** source data and checked against the key's server-side allowed scope. A claim that disagrees with the observed owner, or an observed owner outside the key, is `403`.
