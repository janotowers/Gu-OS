...
**Rate limiting** is per key id, applies to rejected requests as well as accepted ones, and sits outside the signature contract. On Gu OS C2 this is the existing process-local `rateLimit` helper (`apps/web/src/lib/public-rate-limit.ts`), bucketed `legacy-authority:<key_id>`. It is not durable across application instances. A shared durable limiter is a platform prerequisite, not part of this protocol.

**Audit attribution** is `key_id` + `purpose` per request. On Gu OS C2 this is a structured log line (`event: legacy_service_auth`). There is no durable request-audit table in the repository today; adding one is a platform prerequisite and is not invented by this route.
...
- Any endpoint under this contract stops being semantically idempotent in durable state, which is the assumption that lets §8 omit a signature cache. **C2's persist of fail-safe incidents was that event; it was repaired by a durable logical-request identity, not by adding a signature cache.**
