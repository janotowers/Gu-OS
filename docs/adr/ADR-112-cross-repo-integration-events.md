...
**Exactly one of the nine writer families — one of the fourteen individual writers — has a real gap between 7A and 7B**, and on that one the code already issues a second `update_one` on the appointment after the Calendar call, so finalization has a natural home and adds no request. The general two-state mechanism is nonetheless normative for all of them, because a writer whose ordering changes later must not silently start emitting fabricated outcomes.
...
- **Per-appointment ordering** does not need it either, and gets a stronger guarantee without it. **`$push` appends within the single-document update of 7A**, so for one appointment the array order *is* the mutation order — established by MongoDB, precomputed by no client, and unracheable. The drain preserves that order, and may stamp a position at drain time if a consumer ever wants one.

**Stated as a non-claim so no consumer assumes otherwise: C1 does not guarantee delivery ordering across appointments, across entities, or across retries.** `occurred_at` is what a consumer compares if it needs last-writer-wins, and ties are possible. Gu OS's idempotency is keyed on `event_id`, which needs no sequence at all.
...
- **No sequence number is part of the contract** (§7E). Anything that wants per-appointment order reads the pending array's own append order; nothing may depend on a client-precomputed counter, because that computation races.
