...
- **El supervisor de Caso recupera trabajo bloqueado técnicamente (R1 SL-14).** Al compilar la situación, un Work Item en `blocked` se le muestra con **por qué y cuántas veces falló** — motivo, intentos usados y permitidos, y un extracto acotado del último error, enmarcado como dato —, y lleva un alias `[wN]` solo si es recuperable. El juicio devuelve una disposición por alias, y **el vocabulario del cable no comparte ninguna palabra con las posturas**: el modelo escribe `retry_work` o `leave_blocked`, que el parser traduce a los valores de dominio `retry` y `leave` —el registro durable no cambió—. La separación es estructural, no de redacción: mientras ambos vocabularios se solapaban, el modelo escribía la acción de recuperación en `posture` y el juicio se descartaba por incoherente. Un `retry` **reusa la transición `blocked → ready` del propio plano de trabajo** (`retryBlockedItem`), atribuida con `actor: agent` y `source: case_supervisor_retry`. Es el mismo item —sin duplicado—, conserva `attempt_count` y gana exactamente una ventana. El ejecutor lo rechaza si el Work no es `agent_proposed` de ese Caso, si el bloqueo no es `max_attempts_exhausted`, si la capacidad requerida ya no está declarada disponible, si el mismo juicio reportó un `capability_gap`, o si el supervisor **ya otorgó una ventana** a ese item. Ese último límite, además, **ya no se le ofrece al modelo**: los alias cuya ventana está gastada se le declaran como `retry_work` NO DISPONIBLE, del mismo modo —y por la misma razón— que `outboundAvailable` y las capacidades disponibles. Es disponibilidad resuelta desde eventos durables, no una regla que el modelo pueda razonar en contra, y la única disposición que le queda para ese item es `leave_blocked` con el camino que sí carga la responsabilidad. Nada aquí retira Work: el plano de trabajo no tiene esa transición.
