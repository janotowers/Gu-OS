...
| Organización (R1) | `organizations`, `organization_memberships`, `contacts`, `organization_feature_flags`, `organization_tool_secrets`, `organization_policies`, `source_events`, `external_identity_bindings`, `external_conversation_bindings`, `authority_resolutions`, `case_relationships`, `portfolio_presentation_state` |
| Observabilidad | `ai_usage_events` |

Cuatro precisiones, porque la tabla de arriba muestra **schema** y el diagrama muestra **flujos que existen hoy**:
...
- **El supervisor de Caso recupera trabajo bloqueado técnicamente (R1 SL-14).** Al compilar la situación, un Work Item en `blocked` se le muestra con **por qué y cuántas veces falló** — motivo, intentos usados y permitidos, y un extracto acotado del último error, enmarcado como dato —, y lleva un alias `[wN]` solo si es recuperable. El juicio devuelve una disposición por alias, y en el cable el modelo escribe `retry_work` o `leave_blocked`, que el parser traduce de inmediato a los valores de dominio `retry` y `leave` —el registro durable no cambió—. Esa separación de vocabulario existe porque el modelo escribía la disposición en `posture` y el juicio se descartaba por incoherente. **La falla se cerró donde de verdad estaba: en el muestreo, no en la redacción.** Donde la lista de Work ofrece recuperación —y solo ahí—, la respuesta se pide con un **JSON schema estricto** (`nextWorkJsonSchema`), así que un `posture` inválido dejó de estar desalentado para volverse **imposible de generar**. Lo que lo demuestra es que la palabra que el modelo traspapelaba cambiaba con cada intento de prohibirla: `leave_blocked` mientras fue un valor, y `recovery` —el nombre del campo— cuando un booleano dejó de ofrecer valores; tres rondas de instrucciones, un renombre del cable y ese booleano redujeron la falla sin eliminarla, y el booleano se revirtió por medición al no bajar los descartes y empeorar las escenas bloqueadas. Nada de esto relaja el parser: sigue rechazando todo lo que rechazaba, y un proveedor que ignorara el schema no cambiaría ningún veredicto. Fuera de las situaciones bloqueadas la entrada al modelo no cambió en absoluto, `response_format` incluido, que es lo que SA-14.1 exige. Un `retry` **reusa la transición `blocked → ready` del propio plano de trabajo** (`retryBlockedItem`), atribuida con `actor: agent` y `source: case_supervisor_retry`. Es el mismo item —sin duplicado—, conserva `attempt_count` y gana exactamente una ventana. El ejecutor lo rechaza si el Work no es `agent_proposed` de ese Caso, si el bloqueo no es `max_attempts_exhausted`, si la capacidad requerida ya no está declarada disponible, si el mismo juicio reportó un `capability_gap`, o si el supervisor **ya otorgó una ventana** a ese item. Los dos últimos límites, además, **ya no se le ofrecen al modelo**: los alias cuya ventana está gastada, y los que necesitan una capacidad que el Caso ya no declara, se le declaran con el retry NO DISPONIBLE —y en el segundo caso, con el `capability_gap` que debe nombrar—, del mismo modo y por la misma razón que `outboundAvailable` y las capacidades disponibles. Es disponibilidad resuelta desde eventos durables y desde `required_capability` —el mismo campo con el que el ejecutor rechaza—, no una regla que el modelo pueda razonar en contra; la única disposición que le queda para esos items es `leave_blocked` con el camino que sí carga la responsabilidad. **Ambos conjuntos se los dice `supervise.ts`; el juicio nunca los infiere.** Derivar el de capacidades dentro del prompt —leyendo el tipo de trabajo contra la lista disponible, que para el Work `agent_proposed` que este Slice recupera reproduce exactamente al ejecutor— se implementó y se revirtió el mismo día: seis escenarios ya congelados del eval listan el tipo del item bloqueado fuera de esa lista mientras esperan que un retry sea aceptable, de modo que la derivación no les agregaba información sino que redefinía qué miden. Lo que un set congelado significa no es algo que una implementación pueda reinterpretar, y que esos seis describan una situación que `planRecovery` rechazaría quedó registrado como hallazgo de fidelidad del set, no reparado por código que lo lea distinto. Nada aquí retira Work: el plano de trabajo no tiene esa transición.
...
- **La primera ruta sobre el plano de Organización es el Work Portfolio** (`/portfolio`, R1 SL-7; `apps/web/src/lib/work-portfolio/`): una **proyección**, no una segunda fuente de verdad. Lee los Casos de la Organización con el **JWT del propio actor** — las políticas de membresía deciden el conjunto candidato — y los Work Items con `service_role`, llaveados solo por esos ids. Calcula en código los seis predicados *must-surface* (`authority_conflict` es live desde SL-6; `pending_approval` y `unknown_outcome_effect` siguen sin productor hasta SL-9). Escribe solo por mecanismos canónicos: completar un Work Item que espera a una persona (readiness → claim `human` → completion del plano de trabajo, o `review → done`) y decidir una aprobación (`case_approvals`; sin productor antes de SL-9), ambas tras `authorizeOrgAction`; más el estado de presentación personal. Inerte con `relationship_ops` apagado. No ejecuta admisión, resolución ni el supervisor.
- **Orden contextual del Portfolio (R1 SL-12; `apps/web/src/lib/work-portfolio/ranking/`).** Con el flag de Organización `portfolio_contextual_ranking` encendido —apagado por defecto: es el interruptor—, después de autorizar y de los predicados corre **una sola pasada acotada de modelo** por carga: un intento, y uno más solo si el primero se cuelga (12 s), falla en transporte, recibe un 5xx o 429 o devuelve una respuesta inválida —nunca tras otro 4xx—, todo dentro de 20 s. Su entrada es **solo el snapshot autorizado** de SL-7, con alias cortos en lugar de ids (hasta 40 Casos, textos recortados). Ordena Needs Attention y puede admitir situaciones **contextuales**: discrecionales, sin predicado ni obligación, sujetas a posponer/ocultar de la persona, y mostradas solo si el modelo afirmó explícitamente que el Caso necesita a una persona ahora —un guard conservador que solo puede quitar admisiones, nunca prueba de que una esté sustentada— y si cada claim de WHY / WHAT GU NEEDS / WHY NOW cita filas de ese mismo Caso. El piso *must-surface* se re-aplica en código **después** del modelo: presencia, no rango. Nada del modelo se persiste. Si aun así no hay una respuesta válida y completa —nunca se aplica una a medias— rige el orden determinista de SL-7, y la página dice cuál orden muestra. Cada intento va correlacionado a la Organización en `ai_usage_events`.
- **El chat web lee esa misma proyección, solo lectura** (`work_portfolio_read`, `packages/agent/src/tools/work-portfolio-adapters.ts`, cableada en `wire-tool-deps.ts`). Lee con la **sesión del propio usuario** (`actorDb`, que pasan las rutas de chat web) y nunca con `service_role`. La Organización se resuelve de sus membresías activas —ninguna o varias fallan cerrado— y el modelo no puede nombrarla. Sin sesión web (Telegram, cron, heartbeat) se niega. Es opt-in por persona en Settings, visible solo a miembros activos.
- El **legacy gateway sí está cableado** al runtime del agente (`apps/web/src/lib/agent/wire-tool-deps.ts` → `packages/agent/src/tools/legacy-gateway-adapters.ts`): lecturas acotadas por capacidad contra Traditional Gu, sin ninguna ruta de escritura. `legacy_conversation_authority_get` (R1 SL-6) es una lectura semántica adicional, **resolver-facing**: no es tool de modelo y no es un endpoint C6. Compone `gu2.users` (takeover / timestamp) y `gu2.gunumbers` (kill switch) con contribución de provenance por campo. `resolveInteractionAuthority` responde en modo advisory desde esa lectura; mapea el `lead_id` opaco a un Caso por un binding `gu` activo; no escribe `runtime_authority` y no interpreta la ventana de reanudación legacy. `POST /api/legacy/authority` expone esa respuesta al router legacy bajo LegacyServiceAuth v1 (ADR-111), con la Organización y el alcance de owner tomados de la clave frente al owner **observado en servidor**. Un `unknown` / `conflicting` se persiste en `authority_resolutions` solo cuando la Organización y el alcance de owner/source quedan establecidos en servidor; un retry no duplica el incidente, y un fail-safe posterior de esa misma identidad reabre la fila. Un binding Lead→Caso no sustituye la autorización de owner de ADR-111: si el alcance no se puede establecer, C2 responde 403, audita y no persiste. El oráculo de equivalencia (SL-6 T6) observa la ventana de reanudación de cinco minutos de Traditional Gu; no es un invariante de Gu OS y no la comparte el resolver.
...
- `20260914143312_portfolio_presentation_state.sql` — `portfolio_presentation_state` (R1 SL-7): estado de presentación **personal** del Work Portfolio (visto, pospuesto, oculto, fijado) por persona y Caso. Sujeto = un Caso **de esa Organización** por FK compuesta `(subject_id, organization_id)`; `snooze_until` recortado a 14 días desde la escritura por trigger, con la hora de la base. Nunca resuelve una necesidad de negocio y los predicados *must-surface* no lo leen.
- `20260915192651_tool_calls_read_own.sql` — `tool_calls` pasa a lectura propia y no escribible por el usuario (Cycle 3 order 5 / Q9).
...
- **Escritura solo desde el servidor.** Las tablas de Organización tienen políticas de `service_role` para escritura; `organization_tool_secrets` además **no tiene política de lectura para `authenticated`**. La autorización de negocio la hace la aplicación (`authorizeOrgAction`, cuyo vocabulario incluye desde SL-7 `case_approval.decide`: solo `owner` / `org_admin`, nunca por asignación), no el cliente.
- **Una única excepción escribible por el usuario: `portfolio_presentation_state`.** Es presentación personal, no negocio: un `authenticated` lee, inserta y actualiza **solo sus propias filas y solo mientras es miembro activo** de la Organización de la fila (la lectura exige también membresía activa, más estricto que la matriz TD-1); no puede borrar ni re-apuntar una fila a otra persona o sujeto.
...
- **Conversación externa sin identidad implícita.** `external_conversation_bindings` (R1 SL-6) ata un Caso y un contacto de la Organización a una referencia de conversación **opaca** — el `lead_id` legacy es una referencia, nunca la identidad de conversación de Gu OS. La autoridad de conversación (`gu` / `human_active`) vive en esa fila y **queda null por CHECK** en `thread_kind='advisor_wa'`. Distinta de `operational_case_conversation_bindings` (00044). Solo `service_role`. No escribe `runtime_authority`.
- **Resolución de autoridad fail-safe.** `authority_resolutions` (R1 SL-6) guarda solo `unknown` y `conflicting`, con `detected_at`, procedencia e identidad lógica `provider_message_id`. Los miembros de la Organización leen; solo `service_role` escribe. Una observación posterior `gu` / `human_active` cierra el incidente abierto (`resolved_at` / `resolved_as`) **de esa conversación** y no inserta una fila confiada. Un fail-safe posterior de la misma identidad lógica reabre la fila. El Portfolio proyecta solo el incidente **no resuelto** más reciente por Caso en `authority_conflict`. No escribe `runtime_authority`.
...
- **Legacy service (R1 SL-6):** `POST /api/legacy/authority` — autenticación LegacyServiceAuth v1 (ADR-111), purpose `authority-read`, servicio y `sourceSystem` Traditional Gu declarados por el contrato. La Organización y el alcance de owner se resuelven **desde la clave** frente a datos observados en servidor; un claim firmado es solo consistencia. Rate limit de proceso, clasificado antes de autenticar por la forma sintáctica del id (buckets hashed de cardinalidad fija para ids bien formados; la existencia de la clave no elige el bucket); atribución de auditoría `key_id` + `purpose` también tras un fallo de persistencia. Los ids desconocidos no se registran en crudo. La respuesta es advisory: no escribe `runtime_authority` y no suprime réplicas. El body se hashea en crudo; el path firmado es la ruta declarada `/api/legacy/authority` (Next.js no expone el request-target crudo). El middleware de sesión no toca `/api/legacy`.
