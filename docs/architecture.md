...
- **Legacy service (R1 SL-6):** `POST /api/legacy/authority` — autenticación LegacyServiceAuth v1 (ADR-111), purpose `authority-read`, servicio y `sourceSystem` Traditional Gu declarados por el contrato. La Organización y el alcance de owner se resuelven **desde la clave** frente a datos observados en servidor; un claim firmado es solo consistencia. Rate limit de proceso, clasificado antes de autenticar (ids malformados/desconocidos no crean un bucket por valor); atribución de auditoría `key_id` + `purpose` también tras un fallo de persistencia. La respuesta es advisory: no escribe `runtime_authority` y no suprime réplicas. El body se hashea en crudo; el path firmado es la ruta declarada `/api/legacy/authority` (Next.js no expone el request-target crudo). El middleware de sesión no toca `/api/legacy`.
