...
- **El supervisor de Caso recupera trabajo bloqueado técnicamente (R1 SL-14).** Al compilar la situación, un Work Item en `blocked` se le muestra con **por qué y cuántas veces falló** — motivo, intentos usados y permitidos, y un extracto acotado del último error, enmarcado como dato —, y lleva un alias `[wN]` solo si es recuperable. El juicio devuelve una disposición por alias, y en el cable **esa disposición es un booleano**: el modelo escribe `"retry": true` o `false`, que el parser traduce de inmediato a los valores de dominio `retry` y `leave` —el registro durable no cambió—. La razón es estructural, no de redacción: mientras la disposición fue una palabra, el modelo la escribía en `posture` y el juicio se descartaba por incoherente. Tres rondas de instrucciones y un renombre del vocabulario del cable redujeron esa falla sin eliminarla, y la medición del propio renombre explicó por qué —lo que la impulsa es cuánto del vocabulario de la respuesta ocupan esos tokens, así que nombrarlos otra vez, incluso para prohibirlos, la empeoró—. Un booleano no ocupa ninguno: hoy `posture` es **el único enum de texto que el modelo elige**, no queda token que traspapelar, y la falla dejó de ser desalentada para volverse inalcanzable. Un `retry` **reusa la transición `blocked → ready` del propio plano de trabajo** (`retryBlockedItem`), atribuida con `actor: agent` y `source: case_supervisor_retry`. Es el mismo item —sin duplicado—, conserva `attempt_count` y gana exactamente una ventana. El ejecutor lo rechaza si el Work no es `agent_proposed` de ese Caso, si el bloqueo no es `max_attempts_exhausted`, si la capacidad requerida ya no está declarada disponible, si el mismo juicio reportó un `capability_gap`, o si el supervisor **ya otorgó una ventana** a ese item. Los dos últimos límites, además, **ya no se le ofrecen al modelo**: los alias cuya ventana está gastada, y los que necesitan una capacidad que el Caso ya no declara, se le declaran con el retry NO DISPONIBLE —y en el segundo caso, con el `capability_gap` que debe nombrar—, del mismo modo y por la misma razón que `outboundAvailable` y las capacidades disponibles. Es disponibilidad resuelta desde eventos durables y desde `required_capability` —el mismo campo con el que el ejecutor rechaza—, no una regla que el modelo pueda razonar en contra; la única disposición que le queda para esos items es `false` con el camino que sí carga la responsabilidad. Nada aquí retira Work: el plano de trabajo no tiene esa transición.
