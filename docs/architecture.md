...
- **Resolución de autoridad fail-safe.** `authority_resolutions` (R1 SL-6) guarda solo `unknown` y `conflicting`, con `detected_at` y procedencia. Los miembros de la Organización leen; solo `service_role` escribe. El Portfolio proyecta la fila más reciente por Caso en `authority_conflict`. No escribe `runtime_authority`.
...
- **Legacy service (R1 SL-6):** `POST /api/legacy/authority` — autenticación LegacyServiceAuth v1 (ADR-111), purpose `authority-read`. La Organización se resuelve **desde la clave**, nunca desde un claim del payload. La respuesta es advisory: no escribe `runtime_authority` y no suprime réplicas. El body se hashea en crudo; el middleware de sesión no toca `/api/legacy`.
