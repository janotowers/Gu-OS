...
- **El supervisor de Caso recupera trabajo bloqueado técnicamente (R1 SL-14).** Al compilar la situación, un Work Item en `blocked` se le muestra con **por qué y cuántas veces falló** — motivo, intentos usados y permitidos, y un extracto acotado del último error, enmarcado como dato —, y lleva un alias `[wN]` solo si es recuperable. El juicio devuelve `retry` o `leave` por alias; un `retry` **reusa la transición `blocked → ready` del propio plano de trabajo** (`retryBlockedItem`), atribuida con `actor: agent` y `source: case_supervisor_retry`. Es el mismo item —sin duplicado—, conserva `attempt_count` y gana exactamente una ventana. El ejecutor lo rechaza si el Work no es `agent_proposed` de ese Caso, si el bloqueo no es `max_attempts_exhausted`, si la capacidad requerida ya no está declarada disponible, si el mismo juicio reportó un `capability_gap`, o si el supervisor **ya otorgó una ventana** a ese item. Nada aquí retira Work: el plano de trabajo no tiene esa transición.
