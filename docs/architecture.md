...
- **Legacy service (R1 SL-6):** `POST /api/legacy/authority` — autenticación LegacyServiceAuth v1 (ADR-111), purpose `authority-read`. La Organización y el alcance de owner se resuelven **desde la clave** frente a datos observados en servidor; un claim firmado es solo consistencia. Rate limit por `key_id` (aceptadas y rechazadas) con el helper de proceso existente; atribución de auditoría `key_id` + `purpose` en log estructurado. La respuesta es advisory: no escribe `runtime_authority` y no suprime réplicas. El body se hashea en crudo; el path firmado es la ruta declarada `/api/legacy/authority` (Next.js no expone el request-target crudo). El middleware de sesión no toca `/api/legacy`.
