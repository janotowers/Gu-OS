...
| Gu OS-specific mapping       | Operating Domains, Cases, Work Plane, Skills / Tools, Brain, the organization model and paths such as `docs/product/operating-domains/...` and `docs/product/roadmap-increments/...`. These are **concrete examples of how Gu OS applies the method**, not requirements the method imposes. (The earlier `docs/product/initiatives/...` example was retired in v0.4.1 — Sections 4.1 and 23.6 — and that tree no longer exists.) |
...
**A post-run change to a material threshold takes effect prospectively.** Changing such a threshold after runs have been observed is a **new governed threshold decision**, not a reinterpretation of the old one. It does **not** retroactively convert an already-observed failed run into a pass: closing the governed Slice or capability requires **new evidence evaluated under the revised threshold**. How that evidence is produced — which eval framework, whether a holdout is used, how many cases — is an execution choice this Methodology does not prescribe. The invariant is **provenance and non-retroactivity**, not a particular testing implementation. Ordinary engineering thresholds are unaffected and remain under normal engineering authority.
...
**What is genuinely new: material eval-threshold authority (Section 14.1).** Section 14.1 already required model-dependent behavior to meet a rubric and a failure-rate threshold, and Section 12 already kept consequential risk acceptance under human authority. What neither said is what happens when a governing Spec or Slice **requires** such a threshold but no governing artifact has settled its **numerical value**. Section 14.1 now states it: the coding agent may derive, document and freeze a candidate before the first run; observed results may never loosen the bar retroactively; a **material** threshold must be reviewed and ratified by the appropriate human/domain authority before it can close the governed Slice or capability; and ratification after runs must record that sequence honestly rather than being written up as prior approval. It also makes the interaction between the last two explicit: a post-run change to a material threshold is a **new governed threshold decision taking effect prospectively** — it never converts an already-observed failed run into a pass, and closure needs new evidence evaluated under the revised threshold. No eval framework, holdout mechanism or sample size is prescribed; the invariant is provenance and non-retroactivity. **Ordinary engineering thresholds are explicitly excluded** — this clarifies the existing risk-acceptance model rather than adding a gate to every number.
...
**What changes in practice.** A green PR whose consequential decisions are all already authorized no longer waits for a human merge by default. A PR that would enact a decision still owed to a human always does.
...
| AGENTS.md (root)                           | Tracked repo-wide coding-agent operating contract    | Keep concise and stable; it carries monorepo-wide rules and links to canonical docs rather than duplicating them. The v0.4.0-v0.4.2 alignment follow-up is **resolved in v0.4.3** (Sections 11.2, 23.8): the root contract now activates the Development Continuity Loop and its stopping condition by reference instead of copying them. |
