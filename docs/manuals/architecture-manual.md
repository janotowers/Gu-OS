...
- **LegacyServiceAuth inbound (R1 SL-6):** `POST /api/legacy/authority` autentica por HMAC-SHA256 (ADR-111), purpose `authority-read`, servicio y source system Traditional Gu. La Organizacion y el alcance de owner salen de la clave frente al owner observado en servidor. Rate limit de proceso clasificado antes de autenticar; auditoria `key_id` + `purpose` tambien si la persistencia falla. El body se hashea en crudo. Un `unknown` / `conflicting` se persiste de forma idempotente y un fail-safe posterior reabre esa identidad; la respuesta es advisory y no escribe `runtime_authority`. `/api/legacy/events` no existe todavia. El oraculo de equivalencia observa la ventana de reanudacion legacy; no es politica de Gu OS.
