...
| Resolucion de autoridad fail-safe | `unknown` / `conflicting` | Organizacion / Caso | `authority_resolutions` | **Hoy (schema SL-6).** Miembros leen; solo service role escribe. `provider_message_id` es la identidad logica C2. Un incidente abierto se cierra con `resolved_at` / `resolved_as`; el Portfolio solo proyecta no resueltos. No escribe `runtime_authority` |
...
- **LegacyServiceAuth inbound (R1 SL-6):** `POST /api/legacy/authority` autentica por HMAC-SHA256 (ADR-111), purpose `authority-read`. La Organizacion y el alcance de owner salen de la clave frente al owner observado en servidor. Rate limit por `key_id` (aceptadas y rechazadas) con el helper de proceso existente; auditoria `key_id` + `purpose` en log estructurado. El body se hashea en crudo. Un `unknown` / `conflicting` se persiste de forma idempotente; la respuesta es advisory y no escribe `runtime_authority`. `/api/legacy/events` no existe todavia. El oraculo de equivalencia observa la ventana de reanudacion legacy; no es politica de Gu OS.
