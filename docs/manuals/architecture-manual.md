...
La autorizacion de negocio no la hace RLS sola — y tampoco es un solo camino. `packages/db/src/queries/organizations.ts` separa **tres**, y la firma de cada helper delata cual es cual:

- **Mutacion de negocio desde ruta de servidor** — `authorizeOrgAction(db, actorUserId, organizationId, action)`: la guarda determinista que revalida **membresia y rol en el momento de la accion**, nunca heredados de un claim de sesion, de una fila de binding ni de una decision previa. Recibe un **actor** y falla cerrado: el vocabulario de acciones es cerrado y una accion desconocida se **rechaza**, no se permite.
- **Bootstrap / aprovisionamiento / backfill** — `bootstrapOrganizationFromLegacyKey`, `ensureOrganizationMembership`: caminos internos de `service_role` con semantica **acotada a su proposito**, y no llamarlos `authorizeOrgAction` **no los deja sin autorizar**. El bootstrap es solo `service_role` (el privilegio EXECUTE de la funcion SQL esta restringido) y **no crea membresia** como efecto secundario; `ensureOrganizationMembership` es **no-mutante ante conflicto** a proposito, para que un re-run jamas reviva una membresia desactivada ni pise un rol que un administrador cambio.
- **Mutacion explicita del ciclo de vida de la membresia** — `setMembershipStatus`: **esto no es backfill.** Desactivar o reactivar a un miembro es una decision de autoridad que debe tomarse deliberadamente, nunca como efecto secundario. El helper **no recibe actor y no autoriza nada**: es una primitiva de mutacion privilegiada. `service_role` le da el **camino de ejecucion privilegiado**, no la **autoridad de negocio** — esa la debe haber establecido ya la logica orquestadora que lo invoque. Hoy **ningun llamador de runtime lo usa**.
...
| Resolucion de autoridad fail-safe | `unknown` / `conflicting` | Organizacion / Caso | `authority_resolutions` | **Hoy (schema SL-6).** Miembros leen; solo service role escribe. `provider_message_id` es la identidad logica C2. Un incidente abierto se cierra por Caso y conversacion; un fail-safe posterior de la misma identidad reabre la fila. El Portfolio solo proyecta no resueltos. No escribe `runtime_authority` |
...
filas del propio Caso. Después del modelo, el código re-aplica el piso *must-surface* (presencia, no rango);
...
- **LegacyServiceAuth inbound (R1 SL-6):** `POST /api/legacy/authority` autentica por HMAC-SHA256 (ADR-111), purpose `authority-read`, servicio y source system Traditional Gu. La Organizacion y el alcance de owner salen de la clave frente al owner observado en servidor. Rate limit de proceso clasificado antes de autenticar por la forma del id, no por si la clave existe (buckets hashed de cardinalidad fija). Auditoria `key_id` + `purpose` tambien si la persistencia falla; un id desconocido no se registra en crudo. El body se hashea en crudo. Un `unknown` / `conflicting` se persiste solo cuando Organizacion y alcance de owner/source quedan establecidos en servidor; un fail-safe posterior reabre esa identidad. Un binding Lead→Caso no autoriza persistir ni devolver C2. La respuesta es advisory y no escribe `runtime_authority`. `/api/legacy/events` no existe todavia. El oraculo de equivalencia observa la ventana de reanudacion legacy; no es politica de Gu OS.
