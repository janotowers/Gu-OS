...
| **C1** (event forwarding) | **specified** — [ADR-112](../../../adr/ADR-112-cross-repo-integration-events.md) **rev 3**, Proposed | human ratification (§3, decision **B**) — authentication is settled, so this is now C1's only blocker |
...
**B. Ratify the C1 integration-event architecture** — **the one live decision.** [`slice-plan.md`](slice-plan.md) §8 **Q15**, specified in [ADR-112](../../../adr/ADR-112-cross-repo-integration-events.md), now at **rev 3**. Three decisions: **entity identity is not event identity**, **publication must be durable on the producer side**, and — added at rev 3 — **durability is not the same boundary as delivery-readiness**, because an appointment event whose Calendar or Firestore outcome is still unknown must not be sent with those outcomes invented. All three are detailed in §5 below, because they change what you would build.
...
- **Appointments — two boundaries, not one.** A *separate* Mongo outbox collection cannot be transactional with the business write: the Atlas Data API is stateless HTTP and cannot join a multi-document transaction, and no session or transaction usage exists anywhere in the repository. But **every canonical appointment mutation is a write to exactly one appointment document, and Mongo is atomic at the single document, including through the Data API.** So the obligation is recorded **inside the appointment document**, in the same update as the business change — a `pending_integration_events` entry pushed, with no sequence counter. **After that the event cannot be lost.** What it can still be is *incomplete*, which is the second boundary:
  ...
  - **No sequence counter.** ADR-112 §7E removed the `integration_seq` an earlier revision proposed: the client-side read-then-increment-then-push it needed can race, letting two concurrent updates stamp the same value, and nothing in the Data API returns a post-increment value to the `$push` that depends on it. Nothing was needed in its place — the pending array's own non-emptiness is the liveness signal, and **`$push` append order gives per-appointment ordering that no client precomputes.** Drain workers claim an entry with a single-document `findOneAndUpdate` so removing the counter opens no double-delivery hole. **C1 promises no ordering across appointments, entities or retries**; compare `occurred_at` if you need last-writer-wins.
