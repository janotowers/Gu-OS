...
> **Revised:** 2026-09-17 (v0.2) against the contract-precision pass — Technical Plan **v1.19**, [`ADR-111`](../../../adr/ADR-111-legacy-service-auth-v1.md), [`ADR-112`](../../../adr/ADR-112-cross-repo-integration-events.md), [`legacy-source-audit.md`](legacy-source-audit.md) §24. The ask is unchanged in substance and much more precise in detail. **Three statements in v0.1 were wrong or stale and are corrected in place, each marked *Corrected 2026-09-17*.**
...
|---|---|---|
| **TD-13** (`LegacyServiceAuth` v1) | **specified** — [ADR-111](../../../adr/ADR-111-legacy-service-auth-v1.md), Proposed | human ratification (§3, decision **A**) |
| **C1** (event forwarding) | **specified** — [ADR-112](../../../adr/ADR-112-cross-repo-integration-events.md), Proposed | human ratification (§3, decision **B**), plus TD-13 |
...
**C. Name who provisions and rotates the keys** — [`slice-plan.md`](slice-plan.md) §8 **Q16**. Repository evidence establishes the *mechanism* on both sides and not the *accountable owner* of a key spanning both estates. It is one sentence to answer, it is deliberately not a secrets-management redesign, and it is only worth asking once **A** is ratified or heading there. **No secret value was inspected or recorded anywhere in this work.**
...
- **`assignment_change` and `appointment_change`** — a stable `event_id` minted **once** for the logical mutation and **reused across every retry and delivery attempt**. Explicitly **not** the entity id, not `assigned_at`, not a database update timestamp, and not request arrival time. The Mongo appointment `_id` is the **entity** identity; one appointment legitimately has many logical events — creation, reschedule, confirmation, cancellation, status transition — and identifying them by `_id` collapses them into one.
...
- **Appointments — not atomic, and the window is real.** The canonical store is Mongo, reached from the TypeScript services through the **Atlas Data API**, which is stateless HTTP and cannot join a multi-document transaction; PyMongo could in principle open a session, but no session or transaction usage exists anywhere in the repository. So: business write, then integration record, sequentially. **The remaining window is process death between the two, and the consequence is a lost event.** Its bounded recovery is a periodic reconciliation sweep against the last event emitted per appointment. This is **at-least-once with a reconciliation backstop — explicitly not exactly-once and not atomic.** It becomes symmetric with assignment if appointment writes move onto a native driver with sessions; that is named as an upgrade path, not required.
