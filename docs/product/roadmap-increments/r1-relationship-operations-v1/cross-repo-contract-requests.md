...
> **Revised:** 2026-09-17 (**v0.5**) against the contract-precision pass and the three rounds of pre-ratification review that followed it — Technical Plan **v1.22**, [`ADR-111`](../../../adr/ADR-111-legacy-service-auth-v1.md) **rev 2, now RATIFIED**, [`ADR-112`](../../../adr/ADR-112-cross-repo-integration-events.md) **rev 4, still Proposed**, [`legacy-source-audit.md`](legacy-source-audit.md) §24. **The headline for the other team changed: TD-13 authentication is settled and can be built against, and C2 and C6 are no longer blocked on a decision at all.** C1 still is, on ADR-112 alone. The ask is unchanged in substance and much more precise in detail. **Six statements in earlier drafts were wrong or stale and are corrected in place, each marked *Corrected 2026-09-17*** — including one this document got wrong about appointment identity, and two about appointment durability, where rev 1 claimed a guarantee its mechanism could not support and rev 2 then treated a durable obligation as a deliverable event.
...
|---|---|---|
| **TD-13** (`LegacyServiceAuth` v1) | **RATIFIED** — [ADR-111](../../../adr/ADR-111-legacy-service-auth-v1.md) **rev 2**, Accepted direction, 2026-09-17 | Traditional Gu implementation planning; and the provisioning owner (§3, decision **C**) before any real key exists |
| **C1** (event forwarding) | **specified** — [ADR-112](../../../adr/ADR-112-cross-repo-integration-events.md) **rev 4**, Proposed | human ratification (§3, decision **B**) — authentication is settled, so this is now C1's only blocker |
...
Three were raised, **one is settled, and one of the remaining two is deferred by choice — so the live ask is a single decision, B.** Everything else this pass raised was answerable from the repositories and was answered there. All three are recorded in [`slice-plan.md`](slice-plan.md) §8, which owns them.

**A. Ratify TD-13 `LegacyServiceAuth` v1** — **DONE. Ratified as written at rev 2 on 2026-09-17** ([`slice-plan.md`](slice-plan.md) §8 **Q14**), so [ADR-111](../../../adr/ADR-111-legacy-service-auth-v1.md) is **Accepted direction** and TD-13 is no longer TENTATIVE. **This is the sentence to take to the Traditional Gu team: the authentication contract is settled and can be built against.** Two caveats, both narrow. Ratification authorizes no Gu OS code — the verifier, raw-body handling and key registry are SL-5/SL-6 work — and **no key may be provisioned for real traffic until C below names an owner.**
...
**B. Ratify the C1 integration-event architecture** — **the one live decision.** [`slice-plan.md`](slice-plan.md) §8 **Q15**, specified in [ADR-112](../../../adr/ADR-112-cross-repo-integration-events.md), now at **rev 4**. Three decisions: **entity identity is not event identity**, **publication must be durable on the producer side**, and — added at rev 3 — **durability is not the same boundary as delivery-readiness**, because an appointment event whose Calendar or Firestore outcome is still unknown must not be sent with those outcomes invented. Rev 4 then made two concurrency semantics unambiguous: **C1 guarantees no delivery order, not even within one appointment**, and **exactly one finalization per event wins, by compare-and-set rather than by sequencing the writer against the recovery sweep.** All of it is detailed in §5 below, because it changes what you would build.
