...
> **Version:** v1.10  
> **Status:** Approved — governing R1 Slice Plan. **SL-0 and SL-1 are Done** (§6; SL-1 was reopened once after Done and re-closed); **SL-2, SL-3 and SL-4 are READY** (§4, §9) — none of them Planned or Executable; SL-5…SL-13 remain rolling-wave stubs and are NOT READY  
...
**Sequencing, stated precisely.** SL-3 and SL-4 each carry a **case-B dependency on SL-2**, so SL-2 is the sequencing root for the downstream READY work: neither SL-3 nor SL-4 is Executable until its bounded SL-2 prerequisite is **actually satisfied** — satisfaction, not merely SL-2 having started. **SL-2 itself is not currently Executable for a different reason**: it is not Planned and no Cycle or capacity assignment exists, not because it depends on itself. Once the SL-2 prerequisite is satisfied, the downstream READY capacity may become Executable subject to Cycle planning and capacity. All three count toward READY capacity; READY is eligibility for Cycle planning, and nothing here is Planned, Executable, assigned or task-planned.
